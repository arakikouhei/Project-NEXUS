...
ROOT = Path.cwd()
BACKUP_DIR = ROOT / "backups" / "vision_foundation_v1" / datetime.now().strftime("%Y%m%d_%H%M%S")
...
def append_requirement() -> None:
    path = ROOT / "requirements.txt"
    existing = path.read_text(encoding="utf-8") if path.exists() else ""

    if "pillow" not in existing.lower():
        path.write_text(existing.rstrip() + "\nPillow>=10.0.0\n", encoding="utf-8")


def write_vision_tool() -> None:
    write(
...
def patch_tool_manager() -> None:
    ...
    if "from nexus.tools.vision import VisionTool" not in text:
        if "from nexus.tools.safe_search import SafeSearchTool" in text:
            text = text.replace(
        ...
        elif "from nexus.tools.app import AppControlTool" in text:
            text = text.replace(
    ...
    if "self.register(VisionTool())" not in text:
        if "self.register(SafeSearchTool())" in text:
            text = text.replace(
...
def patch_system_prompt() -> None:
    ...
    text = path.read_text(encoding="utf-8")
    marker = "# Vision Foundation Tool v1"
...
NEXUSはローカル画像ファイルの安全確認と基礎分析ができます。
...
def write_docs() -> None:
    write(
        "docs/VISION_FOUNDATION_V1.md",
        r'''
        # Vision Foundation v1

        This is the first local vision layer for Project NEXUS.
        ...
        - object recognition
        - exact plant identification
        - exact mineral identification
        - live camera analysis
        - attack/impact detection

        These are planned for later vision model stages.
        ''',
    )
...
def create_sample_image() -> None:
    try:
        ...
        path = ROOT / "tests" / "assets" / "sample_vision.png"
        path.parent.mkdir(parents=True, exist_ok=True)

        image = Image.new("RGB", (640, 360), (210, 230, 210))
        draw = ImageDraw.Draw(image)
        draw.rectangle((60, 210, 580, 330), fill=(90, 110, 95))
        draw.ellipse((110, 70, 260, 230), fill=(60, 150, 80))
        draw.ellipse((250, 50, 420, 240), fill=(50, 135, 70))
        draw.rectangle((420, 110, 540, 250), fill=(160, 160, 165))
        image.save(path)
...
def main() -> None:
    ...
    print("Vision Foundation v1 applied.")
