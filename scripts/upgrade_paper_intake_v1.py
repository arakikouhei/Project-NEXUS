...
ROOT = Path.cwd()
BACKUP_DIR = ROOT / "backups" / "paper_intake_v1" / datetime.now().strftime("%Y%m%d_%H%M%S")
...
def patch_manager() -> None:
    ...
    if "from nexus.tools.paper_intake import PaperIntakeTool" not in text:
        text = text.replace(
    ...
    if "self.register(PaperIntakeTool())" not in text:
        text = text.replace(
...
def patch_agent_bypass() -> None:
    ...
    if "# PAPER_INTAKE_ROUTING_BYPASS_V1" in text:
        return
...
'''
    ...
    insert = '''        # PAPER_INTAKE_ROUTING_BYPASS_V1
        paper_prefixes = (
            "論文ヘルプ",
            "論文一覧",
            "論文検索:",
...
NEXUSはarXiv APIから安全に論文メタ情報とAbstractを取得し、Knowledge Coreのpapersカテゴリに保存できます。
...
def write_docs() -> None:
    write(
        ...
        v1 uses arXiv API and stores metadata + abstract only.
        ...
        - Abstract-based search and storage only
        """,
    )
